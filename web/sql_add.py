...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "sqlite:///" + config.work_dir + "/web/instance/anime.db"
)
...
def migrate_data():
    ...
    if today_str not in [table_name for table_name in inspector.get_table_names()]:
        TodayAnime.__table__.create(db.engine)

    # 数据迁移
    latest_animes = LatestAnime.query.all()
    for anime in latest_animes:
        today_anime = TodayAnime(
            **{
                column.name: getattr(anime, column.name)
...
def store_data():
    with app.app_context():
        ...
        if os.path.exists(config.work_dir + "/web/instance/anime.db"):
            migrate_data()
